#!/usr/bin/env python3
"""Build a professional Architecture Design Document (.docx) from a constrained
Markdown file.
...
def build_cover(doc, meta):
    ...
    run = title.add_run(meta.get("title", "Architecture Design Document"))
    run.font.color.rgb = ACCENT

    subtitle = doc.add_paragraph()
    subtitle.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = subtitle.add_run(meta.get("subtitle", "Architecture Design Document"))
    run.font.size = Pt(16)
...
def build_header_footer(doc, meta):
    ...
    run = header_p.add_run(meta.get("title", "Architecture Design Document"))
    run.font.size = Pt(9)
    run.font.color.rgb = GREY
